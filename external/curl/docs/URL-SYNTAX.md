...
The path part of an FTP get specifies the file to retrieve and from which
...
FTP servers typically post the user in its "home directory" after login, which
...
The path part of an IMAP get not only specifies the mailbox to list or
...
The path part of a LDAP get can be used to specify the: Distinguished
...
Search for an empty DN and get information about the
...
The path part of a POP3 get specifies the message ID to retrieve. If the
...
## SMB
The path part of a SMB get specifies the file to retrieve and from what
...
The path part of a SMTP get specifies the host name to present during
