 ...
 * The test is successful if the connection can be reused for the next get,
                             ...
                             void *userdata)
{
  ...
  if(data->paused) {
    /* continue to send get body data */
    data->paused = 0;
...
int test(char *URL)
{
  ...
  res = perform_and_check_connections(curl,
    "First get without CURLOPT_KEEP_SENDING_ON_ERROR", 1);
  ...
  res = perform_and_check_connections(curl,
    "Second get without CURLOPT_KEEP_SENDING_ON_ERROR", 1);
  if(res != TEST_ERR_SUCCESS) {
    goto test_cleanup;
  }
  ...
  res = perform_and_check_connections(curl,
    "First get with CURLOPT_KEEP_SENDING_ON_ERROR", 1);
  if(res != TEST_ERR_SUCCESS) {
    goto test_cleanup;
  }
  ...
  res = perform_and_check_connections(curl,
    "Second get with CURLOPT_KEEP_SENDING_ON_ERROR", 0);
  if(res != TEST_ERR_SUCCESS) {
    goto test_cleanup;
  }
