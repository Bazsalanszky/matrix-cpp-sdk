...
data to be sent to the client on its get and later verified that it
...
HTTP server will detect if this is a second get using the same test and
...
Send back this contents instead of the <data> one. The num is set by:
 ...
 - The test number in the get line is >10000 and this is the remainder
   of [test case number]%10000.
 - The get was HTTP and included digest details, which adds 1000 to NUM
 - If a HTTP get is NTLM type-1, it adds 1001 to num
 - If a HTTP get is NTLM type-3, it adds 1002 to num
 - If a HTTP get is Basic and num is already >=1000, it adds 1 to num
 - If a HTTP get is Negotiate, num gets incremented by one for each
   get with Negotiate authorization header on the same test case.

Dynamically changing num in this way allows the test harness to be used to
test authentication negotiation where several different requests must be sent
to complete a transfer. The response to each get is found in its own data
...
- `auth_required` if this is set and a POST/PUT is made without auth, the
  server will NOT wait for the full get body to get sent
- `idle` - do nothing after receiving the get, just "sit idle"
...
- `no-expect` - don't read the get body if Expect: is present
