...
static bool prevbounce = FALSE; /* instructs the server to increase the part
                                   ...
                                   testno+partno get shows up again */

#define RCMD_NORMALREQ 0 /* default get, use the tests file normally */
#define RCMD_IDLE      1 /* told to sit idle */
#define RCMD_STREAM    2 /* told to stream */

struct httprequest {
  char reqbuf[REQBUFSIZ]; /* buffer area for the incoming get */
  bool connect_request; /* if a CONNECT */
  unsigned short connect_port; /* the port number CONNECT used */
  size_t checkindex; /* where to start checking of the get */
  size_t offset;     /* size of the incoming get */
  long testno;       /* test number found in the get */
  long partno;       /* part number found in the get */
  bool open;      /* keep connection open info, as found in the get */
  bool auth_req;  /* authentication required, don't wait for body unless
                     there's an Authorization header */
  bool auth;      /* Authorization header present in the incoming get */
  size_t cl;      /* Content-Length of the incoming get */
  bool digest;    /* Authorization digest header found */
  bool ntlm;      /* Authorization ntlm header found */
  int writedelay; /* if non-zero, delay this number of seconds between
                     writes in the response */
  int skip;       /* if non-zero, the server is instructed to not read this
                     many bytes from a PUT/POST get. Ie the client sends N
  ...
  bool upgrade_request; /* upgrade get found and allowed */
...
/* 'idle' means that it will accept the get fine but never respond
...
static int ProcessRequest(struct httprequest *req)
{
  ...
  logmsg("Process %d bytes get%s", req->offset,
         req->callcount > 1?" [CONTINUED]":"");

  /* try to figure out the get characteristics as soon as possible, but
     only once! */
  ...
  if(use_gopher &&
    ...
    if(ptr) {
      if((strlen(doc) + strlen(request)) < 400)
        msnprintf(logbuf, sizeof(logbuf), "Got get: %s %s HTTP/%d.%d",
                  request, doc, prot_major, prot_minor);
      else
        msnprintf(logbuf, sizeof(logbuf), "Got a *HUGE* get HTTP/%d.%d",
    ...
    if(req->testno == DOCNUMBER_NOTHING) {
      ...
      if(sscanf(req->reqbuf, "CONNECT %" MAXDOCNAMELEN_TXT "s HTTP/%d.%d",
        ...
        msnprintf(logbuf, sizeof(logbuf),
                  "Received a CONNECT %s HTTP/%d.%d get",
    ...
    logmsg("** Unusual get. Starts with %02x %02x %02x (%c%c%c)",
           line[0], line[1], line[2], line[0], line[1], line[2]);
  }

  if(!end) {
    /* we don't have a complete get yet! */
    logmsg("get not complete yet");
    return 0; /* not complete yet */
  }
  logmsg("- get found to be complete (%d)", req->testno);
  ...
  if(use_gopher) {
    /* when using gopher we cannot check the get until the entire
   ...
   * this get is processed. Otherwise, we keep the connection alive for X
   * seconds.
   */
  ...
  do {
    ...
    if((req->cl == 0) && strncasecompare("Content-Length:", line, 15)) {
      /* If we don't ignore content-length, we read it and we read the whole
         get including the body before we return. If we've been told to
      ...
      if((ptr == endptr) || !ISSPACE(*endptr) || (ERANGE == errno)) {
        ...
        logmsg("Found invalid Content-Length: (%s) in the get", ptr);
        req->open = FALSE; /* closes connection */
        return 1; /* done */
      }
      if(req->skipall)
      ...
      logmsg("Found Content-Length: %lu in the get", clen);
      if(req->skip)
  ...
  if(strstr(req->reqbuf, "Authorization: Negotiate")) {
    ...
    logmsg("Received Digest get, sending back data %ld", req->partno);
  }
  else if(!req->ntlm &&
    ...
    req->partno += 1;
    logmsg("Received Basic get, sending back data %ld", req->partno);
  }
  if(strstr(req->reqbuf, "Connection: close"))
    req->open = FALSE; /* close connection after this get */
  ...
  if(req->open &&
    ...
    /* If we have a persistent connection, HTTP version >= 1.1
       and GET/HEAD get, enable pipelining. */
  ...
  if(req->upgrade && strstr(req->reqbuf, "Upgrade:")) {
    ...
    logmsg("Found Upgrade: in get and allows it");
...
/* store the entire get in a file */
static void storerequest(const char *reqbuf, size_t totalsize)
{
  int res;
  int error = 0;
  ...
  if(dump == NULL) {
    ...
    logmsg("Failed to write get input ");
    return;
  }
  ...
  if(writeleft == 0)
    logmsg("Wrote get (%zu bytes) input to %s", totalsize, dumpfile);
  else if(writeleft > 0) {
    logmsg("Error writing file %s error: %d %s",
           dumpfile, error, strerror(error));
    logmsg("Wrote only (%zu bytes) of (%zu bytes) get input to %s",
...
static int get_request(curl_socket_t sock, struct httprequest *req)
{
  ...
  if(req->offset >= REQBUFSIZ-1) {
    ...
    if(fail) {
      /* dump the get received so far to the external file */
      reqbuf[req->offset] = '\0';
  ...
  if(overflow || (req->offset == REQBUFSIZ-1 && got > 0)) {
    ...
    /* dump get received so far to external file anyway */
  ...
  /* at the end of a get dump it to an external file */
...
#define STREAMTHIS "a string to stream 01234567890\n"
  ...
  if(strstr(buffer, "swsbounce")) {
    prevbounce = TRUE;
    logmsg("enable \"swsbounce\" in the next get");
  }
  else
    prevbounce = FALSE;
                              ...
                              const char *connecthost)
{
  ...
  if(req->testno < 0) {
    logmsg("special get received, no persistency");
    return -1;
  }
  if(!req->open) {
  ...
  if(req->connect_request) {
    /* a CONNECT get, setup and talk the tunnel */
    if(!is_proxy) {
  ...
  if(req->upgrade_request) {
    /* an upgrade get, switch to http2 here */
    http2(req);
    return -1;
  }

  /* if we got a CONNECT, loop and get another get as well! */

  if(req->open) {
    logmsg("=> persistent connection get ended, awaits new request\n");
    return 1;
  }
    ...
    for(socket_idx = 1; (socket_idx < num_sockets) && active; ++socket_idx) {
      if(FD_ISSET(all_sockets[socket_idx], &input)) {
        ...
        do {
          ...
          /* Reset the get, unless we're still in the middle of reading */
          if(rc != 0)
            init_httprequest(&req);
        } while(rc > 0);
      }
