...
int SetHTTPrequest(struct OperationConfig *config, HttpReq req, HttpReq *store)
{
  ...
  warnf(config->global, "You can only select one HTTP get method! "
                          ...
                          char *method)
{
  ...
  if(!method)
    ...
    notef(config->global, "Unnecessary use of -X or --get, %s is already "
          "inferred.\n", dflt[req]);
  }
  else if(curl_strequal(method, "head")) {
    warnf(config->global,
          "Setting custom HTTP method to HEAD with -X/--get may not work "
