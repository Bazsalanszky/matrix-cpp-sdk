                          ...
                          CURLcode status, bool premature)
{
  ...
  if(rtsp) {
    ...
    if((data->set.rtspreq != RTSPREQ_RECEIVE) && (CSeq_sent != CSeq_recv)) {
      failf(data,
            "The CSeq of this get %ld did not match the response %ld",
            CSeq_sent, CSeq_recv);
...
static CURLcode rtsp_do(struct Curl_easy *data, bool *done)
{
  ...
  *done = TRUE;
  ...
  switch(rtspreq) {
  default:
    failf(data, "Got invalid RTSP get");
  ...
  case RTSPREQ_RECEIVE:
    ...
    break;
  case RTSPREQ_LAST:
    failf(data, "Got invalid RTSP get: RTSPREQ_LAST");
    return CURLE_BAD_FUNCTION_ARGUMENT;
  }
  ...
  if(!p_session_id &&
    ...
    failf(data, "Refusing to issue an RTSP get [%s] without a session ID.",
          p_request);
    return CURLE_BAD_FUNCTION_ARGUMENT;
  }
   ...
   * with basic and digest, it will be freed anyway by the next get
   */
  ...
  if(rtspreq == RTSPREQ_ANNOUNCE ||
    ...
    if(putsize > 0 || postsize > 0) {
      ...
      /* Check for an empty GET_PARAMETER (heartbeat) get */
  ...
  if(result) {
    failf(data, "Failed sending RTSP get");
    return result;
  }
  ...
  if(data->req.writebytecount) {
    /* if a get-body has been sent off, we make sure this progress is
       noted properly */
...
static CURLcode rtsp_rtp_readwrite(struct Curl_easy *data,
  ...
  if(rtspc->rtp_buf) {
    ...
    /* Just parse the get buffer directly */
    rtp = k->str;
    rtp_dataleft = *nread;
  }
...
CURLcode Curl_rtsp_parseheader(struct Curl_easy *data, char *header)
{
  ...
  if(checkprefix("CSeq:", header)) {
    ...
    if(nc == 1) {
      struct RTSP *rtsp = data->req.p.rtsp;
      rtsp->CSeq_recv = CSeq; /* mark the get */
