...
/*
 * Suppress interactive get for a default password if available.
 */
 ...
 * or non-zero if the session has been post into the session cache.
 */
  ...
  if(SSL_SET_OPTION(primary.sessionid)) {
    ...
    if(!incache) {
      if(!Curl_ssl_addsessionid(data, conn, ssl_sessionid,
        ...
        /* the session has been post into the session cache */
        res = 1;
      }
      else
                                   ...
                                   struct connectdata *conn, int sockindex)
{
  ...
  if(1 != err) {
    ...
    if(SSL_ERROR_WANT_ASYNC == detail) {
      ...
      /* If we e.g. use SSLv2 get-method and the server doesn't like us
