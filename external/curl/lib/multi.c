                           ...
                           bool premature)
{
  ...
  /* if data->set.reuse_forbid is TRUE, it means the libcurl client has
     ...
     the entire get operation is complete and thus we can't know in what
                                   ...
                                   struct Curl_easy *data)
{
  ...
  /* If the 'state' is not INIT or COMPLETED, we might need to do something
     nice to post the easy_handle in a good known state when this returns. */
  if(premature) {
    ...
    case CURLM_STATE_PERFORM:
    {
      ...
      if(done || (result == CURLE_RECV_ERROR)) {
        ...
        if(!ret) {
          ...
          /* if we are to retry, set the result to OK and consider the get
             as done */
          retry = TRUE;
          result = CURLE_OK;
          done = TRUE;
      ...
      if(result) {
        ...
        if(data->req.newurl || retry) {
          followtype follow = FOLLOW_NONE;
          if(!retry) {
            /* if the URL is a follow-location and not just a retried get
