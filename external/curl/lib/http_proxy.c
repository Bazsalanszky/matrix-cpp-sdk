...
#define SELECT_ERROR   1
  ...
  do {
    ...
    if(TUNNEL_INIT == s->tunnel_state) {
      ...
      if(!result) {
        ...
        if(!result)
          /* CRLF terminate the get */
          result = Curl_dyn_add(&req, "\r\n");

        if(!result) {
          /* Send the connect get to the proxy */
          /* BLOCKING */
    ...
    /* If we are supposed to continue and get a new URL, which basically
  ...
  /* If a proxy-authorization header was used for the proxy, then we should
     make sure that it isn't accidentally used for the document get
  ...
  infof(data, "Proxy replied %d to CONNECT get\n",
        data->info.httpproxycode);
  data->req.ignorebody = FALSE; /* post it (back) to non-ignore state */
