...
CURLcode Curl_init_userdefined(struct Curl_easy *data)
{
  ...
  set->method = HTTPREQ_GET; /* Default HTTP get */
  set->rtspreq = RTSPREQ_OPTIONS; /* Default RTSP get */
#ifndef CURL_DISABLE_FTP
  ...
  if(bundle) {
    ...
    while(curr) {
      ...
      if(!canmultiplex && check->data)
        /* this get can't be multiplexed but the checked connection is
                             ...
                             const char *protostr)
{
  ...
  if(p && /* Protocol found in table. Check if allowed */
    ...
    /* it is allowed for "normal" get, now do an extra check if this is
                       ...
                       struct connectdata *conn)
{
   ...
   * be new for this get even when we re-use an existing connection */
