                             ...
                             size_t *nreadp)
{
  ...
  if(conn->handler->protocol&(PROTO_FAMILY_HTTP|CURLPROTO_RTSP)) {
    ...
    if(http->sending == HTTPSEND_REQUEST)
      /* We're sending the HTTP get headers, not the data.
      ...
      if(!k->upload_done) {
        ...
        if((k->exp100 == EXP100_SENDING_REQUEST) &&
          ...
          /* If this call is to send body data, we must take some action:
             We have sent off the full HTTP 1.1 get, and we shall now
 ...
 * as given by the remote server and set up the new URL to get.
 *
                     ...
                     followtype type) /* see transfer.h */
{
  ...
  if(type == FOLLOW_REDIR) {
    if((data->set.maxredirs != -1) &&
      ...
      /* mark the next get as a followed location: */
  ...
  infof(data, "Issue another get to this URL: '%s'\n", data->change.url);
  ...
  switch(data->info.httpcode) {
    /* 401 - Act on a WWW-Authenticate, we keep on moving and do the
       Authorization: XXXX header in the HTTP get code snippet */
    /* 407 - Act on a Proxy-Authenticate, we keep on moving and do the
       Proxy-Authorization: XXXX header in the HTTP get code snippet */
  ...
  case 301: /* Moved Permanently */
     ...
     * method from POST to GET for the subsequent get.  If this
  ...
  case 302: /* Found */
     ...
     *
     * Many webservers expect this, so these servers often answers to a POST
     * get with an error page. To be sure that libcurl gets the page that
     * most user agents would get, libcurl has to force GET.
     *
     * This behavior is forbidden by RFC1945 and the obsolete RFC2616, and
     * can be overridden with CURLOPT_POSTREDIR.
  ...
  case 304: /* Not Modified */
    /* 304 means we did a conditional get and it was "Not modified".
  ...
  case 305: /* Use Proxy */
     ...
     * proxy.  The recipient is expected to repeat this single get
...
/* Returns CURLE_OK *and* sets '*url' if a get retry is wanted.
  ...
  *url = NULL;
  ...
  if((data->req.bytecount + data->req.headerbytecount == 0) &&
    ...
    /* We got no data, we attempted to re-use a connection. For HTTP this
       ...
       it again. Bad luck. Retry the same get on a fresh connect! */
    retry = TRUE;
  ...
  if(k->getheader || !data->set.opt_no_body) {
    ...
    if(writesockindex != -1) {
      /* HTTP 1.1 magic:
         ...
         Thus, we must check if the get has been sent before we set the
      ...
      if((data->state.expect100header) &&
        ...
        if(data->state.expect100header)
          /* when we've sent off the rest of the headers, we must await a
             100-continue but first finish sending the get */
