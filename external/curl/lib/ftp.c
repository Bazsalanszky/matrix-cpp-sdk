 ...
 * AcceptServerConnect()
 *
 * After connection get is received from the server this function is
...
static CURLcode ReceivedServerConnect(struct Curl_easy *data, bool *received)
{
  ...
  *received = FALSE;
  ...
  /* see if the connection get is already here */
  switch(result) {
  case -1: /* error */
    /* let's die here */
 ...
 * get is made. Thus, if an actual transfer is to be made this is where we
 * take off for real.
 */
static CURLcode ftp_state_prepare_transfer(struct Curl_easy *data)
{
                               ...
                               struct connectdata *conn)
{
  ...
  if((ftp->transfer != FTPTRANSFER_BODY) && ftpc->file) {
    /* if a "head"-like get is being made (on a file) */
...
static CURLcode init_wc_data(struct Curl_easy *data)
{
  ...
  wildcard->protdata = ftpwc; /* post it to the WildcardData tmp pointer */
