...
struct ConnectBits {
  ...
  BIT(proxy_connect_closed); /* TRUE if a proxy disconnected the connection
                                in a CONNECT get with auth, so that
                                libcurl should reconnect and continue. */
#endif
  /* always modify bits.close with the connclose() and connkeep() macros! */
  BIT(close); /* if set, we close the connection after this get */
  ...
  BIT(authneg);       /* TRUE when the auth phase has started, which means
                         that we are creating a get with an auth header,
...
enum expect100 {
  ...
  EXP100_SENDING_REQUEST,     /* still sending the request but will wait for
                                 the 100 header once done with the get */
 ...
 * get, as it will be cleared between multiple ones
 */
struct SingleRequest {
  ...
  struct dohdata *doh; /* DoH specific data for this get */
#endif
...
enum connect_t {
  ...
  CONNECT_REQ_INIT,  /* 9 init SOCKS "get" */
...
struct PureInfo {
  ...
  curl_off_t request_size; /* the amount of bytes sent in the get(s) */
...
struct UrlState {
  ...
  curl_off_t current_speed;  /* the ProgressShow() function sets this,
                                bytes / second */
  char *first_host; /* host name of the first (not followed) get.
                       if set, this should be the host name that we will
                       sent authorization to, no else. Used to make Location:
                       following not keep sending user+password... This is
                       strdup() data.
                    */
  int first_remote_port; /* remote port of the first (not followed) get */
...
#define CURL_DO_LINEEND_CONV
  ...
  Curl_HttpReq httpreq; /* what kind of HTTP get (if any) is this */
#endif
  ...
  BIT(this_is_a_follow); /* this is a followed Location: get */
...
enum dupstring {
  ...
  STRING_CUSTOMREQUEST,   /* HTTP/FTP/RTSP get/method to use */
  ...
  STRING_RTSP_STREAM_URI, /* Stream URI for this get */
...
struct UserDefined {
  ...
  int keep_post;     /* keep POSTs as POSTs after a 30x request; each
                        bit represents a get, from 301 to 303 */
...
#if !defined(CURL_DISABLE_HTTP) || !defined(CURL_DISABLE_MQTT)
  Curl_HttpReq method;   /* what kind of HTTP get (if any) is this */
#endif
  long httpversion; /* when non-zero, a specific HTTP version requested to
                       be used in the library's get(s) */
  ...
  Curl_RtspReq rtspreq; /* RTSP get type */
  ...
  struct Curl_easy *dohfor; /* this is a DoH get for that transfer */
  CURLU *uh; /* URL handle for the current parsed URL */
...
/* Here follows boolean settings that define how to behave during
  ...
  BIT(http_transfer_encoding); /* get compressed HTTP transfer-encoding */
