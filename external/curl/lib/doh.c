...
static int doh_done(struct Curl_easy *doh, CURLcode result)
{
  ...
  /* so one of the DOH get done for the 'data' transfer is now complete! */
  dohp->pending--;
  infof(data, "a DOH get is completed, %u to go\n", dohp->pending);
  if(result)
    infof(data, "DOH get %s\n", curl_easy_strerror(result));
                         ...
                         struct curl_slist *headers)
{
  ...
  /* Note: this is code for sending the DoH get with GET but there's still
