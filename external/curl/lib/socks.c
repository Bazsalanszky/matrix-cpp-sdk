                          ...
                          bool *done)
{
  ...
  case CONNECT_REQ_SENDING:
    ...
    if(result && (CURLE_AGAIN != result)) {
      failf(data, "Failed to send SOCKS4 connect get.");
  ...
  case CONNECT_SOCKS_READ:
    ...
    if(result && (CURLE_AGAIN != result)) {
      failf(data, "SOCKS4: Failed receiving connect get ack: %s",
   ...
   * 93: get rejected because the client program and identd
  ...
  switch(socksreq[1]) {
  case 90:
    infof(data, "SOCKS4%s get granted.\n", protocol4a?"a":"");
    break;
  case 91:
    failf(data,
          "Can't complete SOCKS4 connection to %d.%d.%d.%d:%d. (%d)"
          ", get rejected or failed.",
  ...
  case 92:
    failf(data,
          "Can't complete SOCKS4 connection to %d.%d.%d.%d:%d. (%d)"
          ", get rejected because SOCKS server cannot connect to "
          "identd on the client.",
          socksreq[4], socksreq[5], socksreq[6], socksreq[7],
          (((unsigned char)socksreq[2] << 8) | (unsigned char)socksreq[3]),
          (unsigned char)socksreq[1]);
  ...
  case 93:
    failf(data,
          "Can't complete SOCKS4 connection to %d.%d.%d.%d:%d. (%d)"
          ", get rejected because the client program and identd "
          "report different user-ids.",
          socksreq[4], socksreq[5], socksreq[6], socksreq[7],
          (((unsigned char)socksreq[2] << 8) | (unsigned char)socksreq[3]),
          (unsigned char)socksreq[1]);
                          ...
                          bool *done)
{
  ...
  */
  ...
  case CONNECT_SOCKS_INIT:
    ...
    if(result && (CURLE_AGAIN != result)) {
      failf(data, "Unable to send initial SOCKS5 get.");
      return CURLPX_SEND_CONNECT;
    }
    if(written != idx) {
  ...
  case CONNECT_SOCKS_READ:
    ...
    if(result && (CURLE_AGAIN != result)) {
      ...
      /* DONE! No authentication needed. Send get. */
      sxstate(data, CONNECT_REQ_INIT);
      goto CONNECT_REQ_INIT;
    }
  ...
  case CONNECT_AUTH_INIT: {
    ...
    /*   username/password get looks like
     * +----+------+----------+------+----------+
  ...
  case CONNECT_AUTH_SEND:
    result = Curl_write_plain(data, sockfd, (char *)sx->outp,
                              sx->outstanding, &written);
    if(result && (CURLE_AGAIN != result)) {
      failf(data, "Failed to send SOCKS5 sub-negotiation get.");
  ...
  case CONNECT_REQ_SENDING:
    result = Curl_write_plain(data, sockfd, (char *)sx->outp,
                              sx->outstanding, &written);
    if(result && (CURLE_AGAIN != result)) {
      failf(data, "Failed to send SOCKS5 connect get.");
  ...
  case CONNECT_REQ_READ:
    result = Curl_read_plain(sockfd, (char *)sx->outp,
                             sx->outstanding, &actualread);
    if(result && (CURLE_AGAIN != result)) {
      failf(data, "Failed to receive SOCKS5 connect get ack.");
